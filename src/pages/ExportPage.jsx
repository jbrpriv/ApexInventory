...
function PremiumButton({ children, onClick, disabled, style={}, color='#ea580c' }) {
  ...
  return (
    <motion.button onClick={onClick} disabled={disabled}
      ...
      style={{ position:'relative', overflow:'hidden', padding:'12px 20px', borderRadius:10, fontSize:13.5, fontWeight:700, background:disabled?`${color}40`:`linear-gradient(135deg,${color},${color}dd)`, border:'none', color:'white', cursor:disabled?'not-allowed':'pointer', boxShadow:disabled?'none':`0 2px 12px ${color}40`, transition:'background 0.18s', display:'flex', alignItems:'center', justifyContent:'center', gap:8, width:'100%', marginTop:4, fontFamily:'inherit', ...style }}
    >
  ...
  return (
    <div className="fade-in" style={{ padding:'28px 20px', maxWidth:820, margin:'0 auto', minHeight:'100vh' }}>
      <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4 }}>
        ...
        <h1 style={{ fontFamily:'var(--font-display)', fontSize:28, fontWeight:800, color:'white', marginBottom:8, letterSpacing:'-0.5px' }}>Export Accounts</h1>
        <p style={{ color:'rgba(255,255,255,0.4)', fontSize:14, marginBottom:36 }}>Download all account data in your preferred format.</p>
      </motion.div>
        ...
        {exports.map(ex => {
          ...
          return (
            <motion.div key={ex.id} variants={cardV}
              ...
              style={{ background:'#0c0c14', border:`1px solid ${ex.color}25`, borderRadius:16, padding:'26px 24px', display:'flex', flexDirection:'column', gap:16, position:'relative', overflow:'hidden' }}
            >
              {/* Top accent */}
              ...
              <div>
                <div style={{ fontFamily:'var(--font-display)', fontSize:18, fontWeight:700, color:'white', marginBottom:6 }}>{ex.title}</div>
                <div style={{ fontSize:13.5, color:'rgba(255,255,255,0.45)', lineHeight:1.65 }}>{ex.desc}</div>
              </div>

              <div style={{ fontSize:12, color:'rgba(255,255,255,0.3)', background:'rgba(255,255,255,0.04)', padding:'8px 12px', borderRadius:8, fontFamily:'var(--font-mono)', border:'1px solid rgba(255,255,255,0.06)' }}>{ex.meta}</div>
