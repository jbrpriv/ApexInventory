...
function Lv20Badge({ level, rfrBought }) {
  if (rfrBought) return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 9px', borderRadius: 6, background: '#FFFBEB', border: '1px solid #FCD34D', color: '#92400E', fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap' }}>💰 RFR</span>
  );
  if (level >= 20) return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 9px', borderRadius: 6, background: '#ECFDF5', border: '1px solid #6EE7B7', color: '#065F46', fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap' }}>🎮 Made It</span>
  );
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 9px', borderRadius: 6, background: '#F8FAFC', border: '1px solid #E2E8F0', color: '#94A3B8', fontSize: 12, fontWeight: 500, whiteSpace: 'nowrap' }}>— Under 20</span>
  );
}
...
export default function AccountsPage() {
  ...
  /* ── GSAP stagger on card list update ─────────────────────────────────── */
  useEffect(() => {
    if (!cardListRef.current) return;
    const cards = cardListRef.current.querySelectorAll('.account-card');
    if (!cards.length) return;

    /* Only animate when list changes or filters change */
    gsap.fromTo(cards, {
      opacity: 0,
      y: 18,
      scale: 0.97,
  ...
  /* Fire confetti + reload after account is saved */
  const handleSaved = () => {
    setAddOpen(false);
    setShowConfetti(true);
    load();
    setTimeout(() => setShowConfetti(false), 4000);
    toast.success('Account added! 🎉', { duration: 2500 });
  };
  ...
  const CopyBtn = ({ text, id }) => (
    <button type="button" onClick={() => copyText(text, id)}
      ...
      style={{ background: copied === id ? '#ECFDF5' : '#F3F4F6', border: `1px solid ${copied === id ? '#6EE7B7' : '#E5E7EB'}`, color: copied === id ? '#059669' : '#9CA3AF', borderRadius: 5, padding: '2px 7px', fontSize: 11, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s', display: 'inline-flex', alignItems: 'center', gap: 3, minHeight: 'auto', minWidth: 'auto' }}>
  ...
  const EyeBtn = ({ id }) => (
    <button type="button" onClick={() => togglePw(id)}
      className="compact-btn"
      style={{ background: '#F3F4F6', border: '1px solid #E5E7EB', color: '#9CA3AF', borderRadius: 5, padding: '2px 7px', fontSize: 11, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', minHeight: 'auto', minWidth: 'auto' }}>
      {showPws[id]
      ...
      {showConfetti && (
        <div className="confetti-layer">
          <Confetti
            ...
            numberOfPieces={260}
            gravity={0.22}
            colors={['#4F46E5', '#818CF8', '#059669', '#D97706', '#E11D48', '#0284C7']}
          />
        </div>
      )}
      ...
      <div style={{ marginBottom: 24, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
        ...
        <div className="header-actions" style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 10 }}>
          <motion.button
            ...
            whileHover={!syncing ? { boxShadow: '0 2px 8px rgba(79,70,229,0.15)' } : {}}
            whileTap={!syncing ? { scale: 0.97 } : {}}
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '9px 20px', borderRadius: 9, fontSize: 13.5, fontWeight: 600, background: syncing ? '#F3F4F6' : 'white', border: `1px solid ${syncing ? '#E5E7EB' : 'var(--border-md)'}`, color: syncing ? '#9CA3AF' : 'var(--primary)', cursor: syncing ? 'not-allowed' : 'pointer', transition: 'all 0.18s' }}
          >
          ...
          <AnimatePresence>
            {syncResult && !syncing && (
              <motion.div
                className="sync-result-banner"
                initial={{ opacity: 0, y: -8 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -8 }}
                transition={{ type: 'spring', stiffness: 300, damping: 28 }}
                style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', borderRadius: 8, fontSize: 12.5, background: syncResult.failed > 0 ? '#FFFBEB' : '#ECFDF5', border: `1px solid ${syncResult.failed > 0 ? '#FCD34D' : '#6EE7B7'}`, color: syncResult.failed > 0 ? '#92400E' : '#065F46' }}
              >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, padding: '0 20px', borderBottom: '1px solid var(--border)', background: '#FAFBFF' }}>
            ...
            {TABS.map(t => {
              ...
              return (
                <motion.button
                  ...
                  whileTap={{ scale: 0.96 }}
                  style={{ padding: '14px 18px', border: 'none', cursor: 'pointer', fontSize: 13.5, fontWeight: active ? 600 : 500, color: active ? col : 'var(--text3)', background: active ? `${col}0F` : 'transparent', borderBottom: `2px solid ${active ? col : 'transparent'}`, transition: 'all 0.18s', display: 'flex', alignItems: 'center', gap: 7, whiteSpace: 'nowrap' }}
                >
          ...
          {selected.length > 0 && (
            <div className="bulk-bar" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0' }}>
              ...
              {tab === 'edit' && <>
                <button type="button" onClick={() => handleBulkStatus('Unbanned')} style={{ padding: '5px 12px', borderRadius: 6, fontSize: 12, fontWeight: 600, background: '#ECFDF5', border: '1px solid #6EE7B7', color: '#059669', cursor: 'pointer', minHeight: 'auto' }}>✅ Unbanned</button>
                <button type="button" onClick={() => handleBulkStatus('Banned')} style={{ padding: '5px 12px', borderRadius: 6, fontSize: 12, fontWeight: 600, background: '#FFF1F2', border: '1px solid #FECACA', color: '#E11D48', cursor: 'pointer', minHeight: 'auto' }}>🚫 Banned</button>
              </>}
              {tab === 'remove' && <button type="button" onClick={() => setConfirmDel('bulk')} style={{ padding: '6px 14px', borderRadius: 6, fontSize: 12, fontWeight: 700, background: '#E11D48', border: 'none', color: 'white', cursor: 'pointer', boxShadow: '0 2px 8px rgba(225,29,72,0.25)', minHeight: 'auto' }}>Delete {selected.length}</button>}
            </div>
          )}
        </div>
        ...
        {tab === 'add' && (
          <div style={{ padding: '64px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: 400, background: 'linear-gradient(135deg,#F7F8FF,#F0F4FF)' }}>
            <motion.div
              ...
              style={{ width: 64, height: 64, borderRadius: '50%', background: 'linear-gradient(135deg,#4F46E5,#818CF8)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 30, color: 'white', boxShadow: '0 8px 24px rgba(79,70,229,0.3)', marginBottom: 20 }}
            >+</motion.div>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              style={{ padding: '12px 36px', borderRadius: 10, fontSize: 14, fontWeight: 600, background: 'linear-gradient(135deg,#4F46E5,#818CF8)', border: 'none', color: 'white', cursor: 'pointer', boxShadow: '0 4px 16px rgba(79,70,229,0.3)', transition: 'all 0.18s' }}
            >+ Add Account</motion.button>
          ...
          <div style={{ padding: '16px 20px 20px' }}>
            ...
            <div className="filters-row" style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', marginBottom: 16 }}>
              ...
              {[
                ...
                <select key={i} value={f.val} onChange={e => f.set(e.target.value)}
                  style={{ background: 'white', border: '1px solid var(--border-md)', borderRadius: 9, padding: '9px 12px', fontSize: 13.5, color: f.val ? 'var(--text)' : 'var(--text3)', cursor: 'pointer', minWidth: 135, outline: 'none' }}>
                  {f.opts.map(o => <option key={o.v} value={o.v}>{o.l}</option>)}
                </select>
              ))}
              {(search || filterStatus || filterSales) && (
                <button type="button" onClick={() => { setSearch(''); setFilterStatus(''); setFilterSales(''); }} style={{ padding: '8px 14px', borderRadius: 9, background: '#F3F4F6', border: '1px solid #E5E7EB', color: 'var(--text3)', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>✕ Clear</button>
              )}
            </div>
            ...
            {loading ? (
              ...
              <>
                        ...
                        <tr>
                          {(tab === 'edit' || tab === 'remove') && (
                            <th style={{ width: 42, cursor: 'default' }}>
                              <input type="checkbox" checked={selected.length === sorted.length && sorted.length > 0} onChange={toggleAll} style={{ cursor: 'pointer', accentColor: 'var(--primary)', width: 14, height: 14 }} />
                            </th>
                          )}
                          {[
                        ...
                        <AnimatePresence>
                          {sorted.map((acc, i) => (
                            <motion.tr
                              ...
                              style={{ background: selected.includes(acc._id) ? '#EEF2FF' : undefined }}
                            >
                              ...
                              <td>{acc.lastSynced ? <div style={{ fontSize: 11.5, color: acc.syncError ? '#E11D48' : '#059669' }}>{acc.syncError ? '⚠ Error' : new Date(acc.lastSynced).toLocaleDateString('en-PK', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</div> : <span style={{ color: 'var(--text5)', fontSize: 12 }}>Never</span>}</td>
                              <td>
                                <div style={{ display: 'flex', gap: 5 }}>
                                  {tab === 'view'   && <button type="button" onClick={() => setViewAccount(acc)} className="compact-btn" style={{ padding: '5px 12px', borderRadius: 6, fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer', background: 'var(--primary-pale)', color: 'var(--primary)', minHeight: 'auto', minWidth: 'auto' }}>View</button>}
                                  {tab === 'edit'   && <button type="button" onClick={() => setEditAccount(acc)} className="compact-btn" style={{ padding: '5px 12px', borderRadius: 6, fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer', background: '#EFF6FF', color: '#0284C7', minHeight: 'auto', minWidth: 'auto' }}>Edit</button>}
                                  {tab === 'remove' && <button type="button" onClick={() => setConfirmDel(acc._id)} className="compact-btn" style={{ padding: '5px 12px', borderRadius: 6, fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer', background: '#FFF1F2', color: '#E11D48', minHeight: 'auto', minWidth: 'auto' }}>Delete</button>}
                                </div>
                              </td>
                            </motion.tr>
                          ))}
                ...
                {/* ── Mobile cards — GSAP staggered ─────────────────────── */}
                <div ref={cardListRef} className="accounts-mobile" style={{ flexDirection: 'column', gap: 10 }}>
                  {sorted.map(acc => (
                    <div
                      key={acc._id}
                      className="account-card"
                      style={{ border: `1px solid ${selected.includes(acc._id) ? 'var(--primary)' : 'var(--border)'}` }}
                    >
                      ...
                      {/* Email */}
                      <div style={{ fontFamily: 'var(--font-mono)', color: 'var(--text)', marginBottom: 6, wordBreak: 'break-all', fontSize: 13 }}>{acc.accountEmail}</div>
                      ...
                      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 12, alignItems: 'center' }}>
                        ...
                        {acc.apexUsername && <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--primary)', fontSize: 12 }}>@{acc.apexUsername}</span>}
                      </div>

                      {/* Actions */}
                      <div style={{ display: 'flex', gap: 8 }}>
                        {tab === 'view' && (
                          <button type="button" onClick={() => setViewAccount(acc)} style={{ flex: 1, padding: '10px', borderRadius: 9, background: 'var(--primary-pale)', border: 'none', color: 'var(--primary)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>View Details</button>
                        )}
                        {tab === 'edit' && (
                          <button type="button" onClick={() => setEditAccount(acc)} style={{ flex: 1, padding: '10px', borderRadius: 9, background: '#EFF6FF', border: 'none', color: '#0284C7', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Edit</button>
                        )}
                        {tab === 'remove' && (
                          <button type="button" onClick={() => setConfirmDel(acc._id)} style={{ flex: 1, padding: '10px', borderRadius: 9, background: '#FFF1F2', border: 'none', color: '#E11D48', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Delete</button>
                        )}
                      </div>
                    </div>
                  ))}
