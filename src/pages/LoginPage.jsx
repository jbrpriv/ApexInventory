        ...
        <motion.div initial={{ opacity:0, x:-30 }} animate={{ opacity:1, x:0 }} transition={{ delay:0.1, type:'spring', stiffness:200, damping:26 }}>
          <div style={{ display:'inline-flex', alignItems:'center', gap:12, marginBottom:56 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:10, color:'var(--text4)', letterSpacing:2, marginTop:2 }}>ACCOUNT INVENTORY</div>
            </div>
          </div>
          ...
          <p style={{ color:'var(--text4)', fontSize:15, lineHeight:1.7, maxWidth:320 }}>
      ...
      <div className="login-form-wrap" style={{ position:'relative', zIndex:2, display:'flex', alignItems:'center', justifyContent:'center', padding:'24px 20px', width:'100%', maxWidth:480, flexShrink:0 }}>
        ...
        >
          ...
          <div style={{ marginBottom:28 }}>
            ...
            <p style={{ color:'var(--text4)', fontSize:14 }}>Sign in to access your dashboard</p>
          </div>

          <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:18 }}>
            {/* Username */}
            <motion.div custom={0} variants={fieldV} initial="hidden" animate="visible">
              <label style={{ display:'block', fontSize:11, fontWeight:700, color:'var(--text4)', letterSpacing:1, textTransform:'uppercase', marginBottom:7 }}>Username</label>
            ...
            <motion.div custom={1} variants={fieldV} initial="hidden" animate="visible">
              <label style={{ display:'block', fontSize:11, fontWeight:700, color:'var(--text4)', letterSpacing:1, textTransform:'uppercase', marginBottom:7 }}>Password</label>
              <div style={{ position:'relative' }}>
                <div style={{ position:'absolute', left:13, top:'50%', transform:'translateY(-50%)', color:'var(--text4)', pointerEvents:'none' }}>
                ...
                <input type={showPw?'text':'password'} value={form.password} onChange={set('password')} placeholder="Your password" autoComplete="current-password" style={{...inpStyle,paddingRight:44}}
                  ...
                  onBlur={e=>{e.target.style.borderColor='var(--border)';e.target.style.boxShadow='none';e.target.style.background='var(--surface2)';}}
                />
                <button type="button" onClick={()=>setShowPw(p=>!p)} style={{ position:'absolute', right:12, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', color:'var(--text4)', cursor:'pointer', padding:4, display:'flex', alignItems:'center', minHeight:'auto', minWidth:'auto' }}>
                  {showPw
          ...
          <p style={{ textAlign:'center', marginTop:20, color:'var(--text4)', fontSize:12 }}>Apex Legends Account Manager</p>
        </motion.div>
      </div>
    </div>
  );
