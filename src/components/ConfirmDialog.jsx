  ...
  return ReactDOM.createPortal(
    ...
    >
      <motion.div initial={{ scale:0.9, opacity:0, y:20 }} animate={{ scale:1, opacity:1, y:0 }} transition={{ type:'spring', stiffness:300, damping:28 }}
        ...
        style={{ background:'#0c0c14', border:`1px solid ${danger?'rgba(244,63,94,0.3)':'rgba(234,88,12,0.3)'}`, borderRadius:20, width:'100%', maxWidth:380, overflow:'hidden', boxShadow:'0 24px 64px rgba(0,0,0,0.8)' }}
      >
        ...
        <div style={{ padding:'28px 26px 20px', textAlign:'center' }}>
          ...
          <div style={{ fontFamily:'var(--font-display)', fontWeight:700, fontSize:18, color:'white', marginBottom:8 }}>{title}</div>
          <div style={{ fontSize:14, color:'rgba(255,255,255,0.45)', lineHeight:1.65 }}>{message}</div>
        </div>
        <div style={{ padding:'0 26px 26px', display:'flex', gap:10 }}>
          <button onClick={onCancel} style={{ flex:1, padding:'11px', borderRadius:10, fontSize:14, fontWeight:600, background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', color:'rgba(255,255,255,0.7)', cursor:'pointer', transition:'all 0.15s' }}
            onMouseEnter={e=>e.currentTarget.style.background='rgba(255,255,255,0.1)'}
            onMouseLeave={e=>e.currentTarget.style.background='rgba(255,255,255,0.06)'}
          >Cancel</button>
          <button onClick={onConfirm} style={{ flex:1, padding:'11px', borderRadius:10, fontSize:14, fontWeight:700, background:danger?'#f43f5e':'#ea580c', border:'none', color:'white', cursor:'pointer', transition:'all 0.2s', boxShadow:danger?'0 2px 12px rgba(244,63,94,0.4)':'0 2px 12px rgba(234,88,12,0.4)' }}
